...
sktx_network_path=/etc/sysconfig/network-scripts/ifcfg-eth0
...
bootstrap_set_global_config() {
    ...
    # Network variables
    [ -f "$sktx_network_path" ] && . $sktx_network_path
    sktx_node_host_ip=$IPADDR
    sktx_node_host_name=$(hostname)

    # Deploy variables
    sktx_backup_rotate=$(bootstrap_get_config "backup_rotate")
    ...
    sktx_backup_rotate=${sktx_backup_rotate:-4}
...
bootstrap_get_bootstrap() {
    ...
    bootstrap_print ">>>> Download: bootstrap - $_template" "black"
...
bootstrap_template_in() {
    ...
    if [ -f "$_files_path" ]; then
        ...
        do
            ...
            if [ "$package_name" == "$_template" ]; then
                G_RET_VAL=0
                break
            fi
        done
...
bootstrap_get_files() {
    ...
    bootstrap_print ">>>> Download: task files - $_template" "black"
    _bootstrap_get "$_files_url" "$_files_path"
    bootstrap_print

    if [ -f "$_files_path" ]; then
        bootstrap_print ">>>> Download: sync files - $_template" "black"
        for stat in $(bootstrap_files $_files_path)
        do
            bootstrap_parse_files "$stat"
...
bootstrap_install_files() {
    ...
    local _link_src= _link_dst= _link_dir= _link_name=
    ...
    bootstrap_print ">>>> Localize: sync files - $_template" "black"
    ...
    if [ -f "$_files_path" ]; then
        ...
        do
            ...
            # Check node if $stat_node_host is true
            [ -n "$stat_node_host" ] && [[ ! "$stat_node_host" =~ ^($sktx_node_host_ip|$sktx_node_host_name)$ ]] && continue
            ...
            # Install files(file, directory, link)
            if [ "$stat_type" == "f" ]; then
                _entry_dir=$(dirname ${stat_path})
                _entry_dest="${_entry_path}${stat_path}"
                _entry_path="${_entry_path}${_entry_dir}"
                ...
                bootstrap_print "[$G_RET_STR] f:$stat_path" "white"
                ...
                _link_src=${stat_path%%@*}
                _link_dst=${stat_path##*@}
                _link_dir=$(dirname $_link_dst)
                _link_name=$(basename $_link_dst)

                pushd $_link_dir &> /dev/null

                [ -h "$_link_name" ] && unlink $_link_name
                _bootstrap_link_force $_link_src $_link_name
                bootstrap_print "[$G_RET_STR] l:$_link_dst" "white"
                [ -n "$stat_uid" -a "$stat_uid" != "-" ] && chown -h $stat_uid $_link_name
                [ -n "$stat_gid" -a "$stat_gid" != "-" ] && chown -h :$stat_gid $_link_name
...
bootstrap_sync_file() {
    ...
    bootstrap_install_files $_template
    return $G_RET_VAL
}

bootstrap_sync_jobs() {
    local _template=$1 _jobs_path= _job=
    _jobs_path="$sktx_root/control/module"
    _jobs=$(echo ${_jobs_path}/*)

    [ "$_jobs" == "${_jobs_path}/*" ] && return 0

    bootstrap_print ">>>> Modules: exec jobs - $_template" "black"
    for _job in $_jobs
    do
        bootstrap_print "$ $_job" "yellow"
        $_job
        _bootstrap_remove_file "$_job"
    done
    bootstrap_print
...
_bootstrap_backup_rotate() {
    local _path=$1 _files= _count= _end= i=

    _files=($(echo ${_path}.*))
    _count=${#_files[@]}
    _end=$((_count - sktx_backup_rotate))

    [ $sktx_backup_rotate -ge $_count ] && return 0

    for ((i=0; i < _end; i++))
    do
        _bootstrap_remove_file "${_files[$i]}"
    done
}
...
_bootstrap_backup_file() {
    ...
    if [ "$G_RET_VAL" == "1" ]; then
        ...
        bootstrap_print "[$G_RET_STR] b:$_repository_path/${repofile}.${ntime}" "yellow"
        _bootstrap_backup_rotate "$_repository_path/${repofile}"
    fi
